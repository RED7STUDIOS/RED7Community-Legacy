...
if($_SERVER["REQUEST_METHOD"] == "POST"){
	...
	if(empty($username_err) && empty($password_err) && empty($confirm_password_err)){
		...
		if($stmt = mysqli_prepare($link, $sql)){
			...
			// Attempt to execute the prepared statement
			if(mysqli_stmt_execute($stmt)){
				$data = file_get_contents($API_URL. '/user.php?key='. $API_KEY. '&api=getbyname&name='. $username);
...
<html lang="en">
	<head>
		...
		<link href="<?php echo $STORAGE_URL; ?>/assets/css/sidebar.css" rel="stylesheet">

		<link href="<?php echo $STORAGE_URL; ?>/assets/css/style.css" rel="stylesheet">
	...
	<body>
		...
		<div class="page-content-wrapper">
				...
				<main class="col-md-9 form-signin">
					<?php
					$sql = "SELECT id, name, content FROM site_info";
						...
						while($row = mysqli_fetch_assoc($result)) {
							if ($row["name"] == "registration") {
								if ($row["content"] == "on") {
									...
									echo '<div class="form-group '; echo (!empty($username_err)) ? 'has-error' : '' .'">';
									...
									echo '</div>';
									echo '<div class="form-group '; echo (!empty($password_err)) ? 'has-error' : ''. '">';
									...
									echo '</div>';
									echo '<div class="form-group '; echo (!empty($confirm_password_err)) ? 'has-error' : '' .'">';
