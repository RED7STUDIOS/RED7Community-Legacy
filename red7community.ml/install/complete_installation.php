...
################################################################################
	...
	if($passed_step == 5){
		...
		$admin_password 	 = password_hash(isset($_SESSION['admin_password']) ? stripslashes($_SESSION['admin_password']) : '', PASSWORD_DEFAULT);
					...
					if(EI_CHECK_DB_MINIMUM_VERSION && (version_compare($db->GetVersion(), EI_DB_MINIMUM_VERSION, '<'))){
						...
						if($sql_dump != ''){
							if(false == ($db_error = apphp_db_install($sql_dump_file))){
								...
								$config_file = str_replace('<URL>', $_SERVER['SERVER_NAME'], $config_file);
								
								chmod(EI_CONFIG_FILE_PATH, 0777);
								$f = fopen("../assets/config.php", 'w+');
...
<head>
    ...
    <meta name="generator" content="RED7Community Installer">
