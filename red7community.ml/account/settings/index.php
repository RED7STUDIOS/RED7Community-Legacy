...
<div class="page-content-wrapper">
    ...
    <main class="col-md-9">
        ...
        <p>Display Name: <b><?php if ($your_displayname == null || $your_displayname == "") {
        ...
        <a href="/account/settings/change-password.php" class="btn btn-dark" role="button"><i class="fas fa-key"></i>
            Change Password</a>
        ...
        <fieldset>
            ...
            <form method="post" action="/ajax/process.php"
                  onSubmit="return ajaxSubmit(this);">
                <input maxlength="12" type="text" name="value" style="width: 100%;"
                       value="<?php $sql = "SELECT displayname FROM users WHERE id=" . $_SESSION['id'];
            ...
            <form method="post" action="/ajax/process.php"
                  onSubmit="return ajaxSubmit(this);">
                            <textarea maxlength="200" type="text" name="value"
                                      style="width: 100%; border: 0 none white; overflow: hidden; padding: 0; outline: none; background-color: #D0D0D0;"><?php $sql = "SELECT description FROM users WHERE id=" . $_SESSION['id'];
                                $result = mysqli_query($link, $sql);
                                if (mysqli_num_rows($result) > 0) {
                                    while ($row = mysqli_fetch_assoc($result)) {
        ...
        <fieldset>
            ...
            <form method="post" action="/ajax/process.php"
                ...
                <input type="text" name="value" style="width: 100%;"
                       value="<?php $sql = "SELECT email FROM users WHERE id=" . $_SESSION['id'];
                       $result = mysqli_query($link, $sql);
                       if (mysqli_num_rows($result) > 0) {
                           while ($row = mysqli_fetch_assoc($result)) {
                               echo $row['email'];
