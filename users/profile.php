...
if (!str_contains($data, "This user doesn't exist or has been deleted"))
{
	...
	$data_avatar = file_get_contents($API_URL. '/avatar.php?key='. $API_KEY. '&api=getbyid&id='. $_GET['id']);
...
<html lang="en">
	<head>
		...
		<script src="<?php echo $STORAGE_URL; ?>/assets/js/relation.js"></script>
					...
					<div class="d-flex align-items-center border-bottom">
						...
						<div id="userList"><?php
							...
							if (isset($_SESSION['loggedin']))
							{
							  ...
							  foreach ($users as $id=>$name) { if ($id == $_GET['id'] && $id != $uid) {
								echo "<div class='urow'>";
								...
								// STRANGERS
								  ...
								  echo "<a class='btn btn-success' onclick=\"relate('add', $id)\">Add Friend</a>";
								}
								if (in_array($id, json_decode($your_following, true)))
								{
									echo "&nbsp;<a class='btn btn-danger' href='/users/actions.php?action=unfollow&id={$id}'>Unfollow</a>";
								}
								else
								{
									echo "&nbsp;<a class='btn btn-success' href='/users/actions.php?action=follow&id={$id}'>Follow</a>";
								}
	...
	<script type="module">
		...
		function main() {
			...
			<?php
				$data = file_get_contents($API_URL. '/catalog.php?key='. $API_KEY. '&api=getitembyid&id='. $face);
				...
				$data_shirt = file_get_contents($API_URL. '/catalog.php?key='. $API_KEY. '&api=getitembyid&id='. $shirt);
				...
				$data_pants = file_get_contents($API_URL. '/catalog.php?key='. $API_KEY. '&api=getitembyid&id='. $pants);
			...
			// LEFT ARM MODEL
			{
				...
				foreach(json_decode($hats, true) as $hat) {
					$data = file_get_contents($API_URL. '/catalog.php?key='. $API_KEY. '&api=getitembyid&id='. $hat);
